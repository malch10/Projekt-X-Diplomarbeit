...
for root, dirs, files in os.walk(basis_pfad):
    for dir in dirs:
        ...
        index_names = pd.read_csv(pfad + '/TempPSim' + dir_wo + '_coorX.txt', header=None).squeeze()
        column_names = pd.read_csv(pfad + '/TempPSim' + dir_wo + '_coorY.txt', header=None).squeeze()
